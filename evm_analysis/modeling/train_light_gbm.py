...
def build_prob_matrix(model, X_input):
    # MultiOutputClassifier.predict_proba returns a list of arrays (one for each label)
    # where each array has shape (n_samples, 2) [prob_0, prob_1] or (n_samples, 1) [prob_0]
    ...
    # model.estimators_ contains the individual LGBM classifiers
    for i, probs in enumerate(proba_outputs):
        # MultiOutputClassifier might not have classes_ at the top level, 
        # so we get classes_ from the underlying estimator for this label
        estimator_classes = model.estimators_[i].classes_
        
        if probs.shape[1] == 1:
            # Only one class present in training data for this label
            class_val = estimator_classes[0]
            if class_val == 1:
                prob_matrix[:, i] = 1.0
            else:
                prob_matrix[:, i] = 0.0
        else:
            # Typical case: 2 classes [0, 1]
            # Find the index of class 1
            class1_idx = np.where(estimator_classes == 1)[0]
            if len(class1_idx) > 0:
                prob_matrix[:, i] = probs[:, class1_idx[0]]
            else:
                prob_matrix[:, i] = 0.0
    ...
    for top_k in TOP_K_SLOTS_LIST:
        ...
        for fc in FEATURE_CONFIGS:
            ...
            base_lgbm = LGBMClassifier(
                ...
                n_jobs=4,              # Use multiple CPU cores for each LGBM model
                verbose=-1             # Suppress output
            )
            # Wrap with MultiOutputClassifier to support multi-label
            model = MultiOutputClassifier(base_lgbm, n_jobs=4)
            model.fit(X_train, y_train)

            model_path = os.path.join(MODEL_DIR, f'evm_model_lgbm_k{top_k}_{f_name.replace("+", "_")}.pkl')
            save_data = {
                'model': model,
                'le_to': le_to,
            ...
            # Only save the most complete model as evm_model_lgbm_v1.pkl
            if top_k == 1000 and f_name == 'to+sel+params+codehash+from+value':
                joblib.dump(save_data, os.path.join(MODEL_DIR, 'evm_model_lgbm_v1.pkl'))
